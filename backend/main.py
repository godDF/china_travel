...
from chinatravel.data.load_datasets import NpEncoder
...
async def extract_requirements(llm: Deepseek, conversation: list[dict], current_extracted: dict) -> dict:
...
def _format_multi_plan(plan: dict, req: dict) -> str:
    """Format multiple plans with ranking by budget proximity."""
    plans = plan.get("plans", [])
    budget = req.get("budget")

    lines = [f"为你找到 {len(plans)} 套旅行方案！", ""]
...
async def api_chat(req: ChatRequest):
    ...
    extraction = await extract_requirements(llm, conversation, session["extracted"])
